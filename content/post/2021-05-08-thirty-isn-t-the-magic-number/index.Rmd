...
## First Example: $n\geq 30$ Isn't Necessary 
...
Informally speaking, a $\text{Uniform}(0,1)$ random variable is equally likely to take on any continuous value in the range $[0,1]$. Here's a histogram of 1000 random draws from this distribution:
```{r}
set.seed(12345) # ensures that you'll get the same random draws as I did
...
But if we *average* even a relatively small number of $\text{Uniform}(0,1)$ draws, the result will be *extremely close* to normality. To see that this is true, we'll carry out a simulation in which we draw $n$ $\text{Uniform}(0,1)$ RVs, calculate their sample mean, and store the result. Repeating this a large number of times allows us to approximate the sampling distribution of $\bar{X}_n$. I'll start by writing a function `get_unif_sim` that takes a single argument `n`. This function returns the sample mean of `n` $\text{Uniform}(0,1)$ draws:
```{r}
...
$$-->
If $X \sim \text{Uniform}(0,1)$, then $\mathbb{E}(X) = 1/2$ and $\text{Var}(X) = 1/12$, which follows from $\mathbb{E}[X^2]=1/3$ and the definition of variance.^[If you're taking introductory probability and statistics, filling in the missing details for these calculations would be an excellent homework problem!] For $n = 10$, 
$$
...
## Second Example: $n\geq 30$ Isn't Sufficient

Now suppose that $n = 100$ and $X_1, \dots X_n \sim \text{iid Bernoulli}(1/60)$. What is the CDF of $\bar{X}_n$? Rather than approximating the answer to this question by simulation, as we did in the uniform example from above, we'll work out the *exact* result and compare it to the approximation provided by the CLT. If $X_1, \dots, X_n \sim \text{iid Bernoulli}(p)$, then by definition the sum $S_n = \sum_{i=1}^n X_i$ follows a Binomial$(n,p)$ distribution. The probability mass function and CDF of this distribution are available in R via the `dbinom()` and `pbinom()` commands. So what about $\bar{X}_n$? Notice that
$$
...
plot(x, P_x_bar, type = 'h', xlim = c(0, 0.1), ylab = 'pmf of Xbar', lwd = 2, col = 'blue')
```
...
legend('topleft', legend = c('Exact', 'CLT'), col = c('blue', 'red'), lty = 1:2, lwd = 2)
```
...
```{r}
# Make 5000 draws for phat, each based on a sample of n = 100 observations
# with p = 1/60
set.seed(54321)
