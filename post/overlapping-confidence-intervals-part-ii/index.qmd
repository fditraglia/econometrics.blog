...
series: 'Overlapping Confidence Intervals'
series-label: 'Part II'
series-part-title: 'Correlated estimates and the law of cosines'
...
In my earlier post on [Overlapping Confidence Intervals](/post/overlapping-confidence-intervals/) I asked what we can learn from the overlap, or lack thereof, between confidence intervals for two population means constructed using independent samples.
...
In [Part I](/post/overlapping-confidence-intervals/) we found a way to rephrase our statistical problem about overlapping confidence intervals as a familiar geometry problem involving *right triangles*.
...
As long as the two estimators are uncorrelated and (approximately) normally distributed, the results from [Part I](/post/overlapping-confidence-intervals/) apply to the individual intervals for $\alpha$ and $\beta$ versus the interval for the difference $\alpha-\beta$. 
...
$$
So if we *had* the raw data for treatments A and B we'd be back to a familiar independent samples comparison of means, as in [Part I](/post/overlapping-confidence-intervals/).
...
Using the [properties of variance and covariance](/post/random-variables-cheatsheet/),
$$
...
$$
If $\rho = 0$ this reduces to our formula from [Part I](/post/overlapping-confidence-intervals/):
...
create_triangle <- function(se_a, se_b, rho, title_label) {
  ...
  ggplot() +
    ...
    geom_segment(aes(x = p2[1], y = p2[2], xend = p3[1], yend = p3[2]),
                 color = econblog_accent, linewidth = 2) +
    # Angle arc
    geom_path(data = arc_df, aes(x = x, y = y), 
              color = econblog_warm, linewidth = 1) +
    # Vertices
    ...
    annotate("text", 
             x = if(rho >= 0) mid_left_x - 2.3 else mid_left_x - 2.8,
    ...
    annotate("text", 
             ...
             size = 4, color = econblog_accent, fontface = "bold") +
    # Angle label
    annotate("text", 
             x = arc_radius * 1.3, 
             y = arc_radius * sin(theta/2) * 0.7,
             label = sprintf("%d°", round(theta * 180/pi)),
             size = 4, color = econblog_warm) +
    # Title
...
Just as we argued in [Part I](/post/overlapping-confidence-intervals/), the individual confidence intervals for $\alpha$ and $\beta$ overlap precisely when $(\hat{\alpha} - \hat{\beta})/z < \text{SE}(\hat{\alpha}) + \text{SE}(\hat{\beta})$ and there is a significant difference between the two parameters when $(\hat{\alpha} - \hat{\beta})/z > \text{SE}(\hat{\alpha} - \hat{\beta})$.
