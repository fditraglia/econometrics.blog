...
series-label: 'Part I'
series-part-title: 'Why controlling for an instrument backfires'
series-position: 'Part I of II'
categories:
- econometrics
- causal inference
tags:
...
The logic of two-stage least squares is that regressing $Y$ on the "good" variation, $\pi_1 Z$ allows us to recover $\beta$, the causal effect of interest.^[See [this blog post](/post/three-ways-of-thinking-about-instrumental-variables/) for more discussion.]
...
$$
where $\tilde{X}$ is defined as the *residual* in another population linear regression: the regression of $X$ on $Z$.^[This is a special case of the so-called [FWL Theorem](https://en.wikipedia.org/wiki/Frisch%E2%80%93Waugh%E2%80%93Lovell_theorem), although I'd argue that we should call it "Yule's Rule" since [George Udny Yule](https://en.wikipedia.org/wiki/Udny_Yule) was arguably the first person to popularize it, decades before F, W, or L.]
...
In our simulation, $\widehat{\beta}_{X.Z}$ gave a *worse* estimate of $\beta$ than $\widehat{\beta}_\text{OLS}$.
...
I am pleased to inform you that the answer is yes!

In an [earlier post](/post/three-ways-of-thinking-about-instrumental-variables/) I described the **control function** approach to instrumental variables regression.
...
A common piece of advice for avoiding bad controls is to only include control regressors that are correlated with $X$ and $Y$ but are *not themselves* caused by $X$.
The example in this post shows that this advice **is wrong**.
