...
date: '2024-08-02'
subject: 'Causal inference & identification'
execute:
  message: false
  warning: false
categories:
...
$$
where $V$ is the population linear regression error term so that, [by construction](/post/why-econometrics-is-confusing-part-1-the-error-term/), $\mathbb{E}(V) = \mathbb{E}(XV) = 0$. Notice that I've called the coefficients in this regression $\alpha$ rather than $\beta$. That's because they will *not in general coincide* with the conditional mean function from above, namely $\mathbb{E}(Y|D, X) = \beta_0 + \beta_1 D + \beta_2 X + \beta_3 DX$. In particular, the regression of $Y$ on $D$ and $X$ without an interaction will *only* coincide with the true conditional mean function if $\beta_3 = 0$. 

So what, if anything, can we say about $\alpha_1$ in relation to the ATE? By [Yule's Rule](https://en.wikipedia.org/wiki/Frisch%E2%80%93Waugh%E2%80%93Lovell_theorem)^[Call it "Frisch-Waugh-Lovell" if you must, but I will continue trying to [make fetch happen](https://ideas.repec.org/p/arx/papers/2307.00369.html).] we have
$$
...
And now the easy way, using the two regressions described above^[If you're rusty on R's formula syntax, you may find my [cheat sheet](/post/the-r-formula-cheatsheet/) helpful.]
```{r}
...
$$
where $U \equiv Y - \mathbb{E}(Y|D, X)$ satisfies $\mathbb{E}(U|D,X) = 0$ [by construction](/post/why-econometrics-is-confusing-part-1-the-error-term/). This allows us to write
$$
\begin{aligned}
...
\end{aligned}
$$
So what about that pesky $\text{Cov}(U,D|X)$ term? By the law of iterated expectations this turns out to equal zero, since 
$$
\begin{aligned}
...
\end{aligned}
$$
and, again, $\mathbb{E}(U|D,X) = 0$ [by construction](/post/why-econometrics-is-confusing-part-1-the-error-term/). So we're left with 
$$
\alpha_1 = \frac{\mathbb{E}[\text{Cov}(Y, D|X)]}{\mathbb{E}[\text{Var}(D|X)]} = \frac{\mathbb{E}[\text{Var}(D|X)(\beta_1 + \beta_3 X)]}{\mathbb{E}[\text{Var}(D|X)]}.
$$
