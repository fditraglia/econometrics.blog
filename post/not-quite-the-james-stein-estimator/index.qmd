...
op <- par(mfrow = c(2, 2), mar=c(3,3,2,1), mgp=c(2,.7,0), tck= -.01, oma = c(0,0,0,0))
...
$$
This estimator dominates the ML estimator when $p \geq 3$ in that  
$$
...
\end{aligned}
$$
Since $c^2/(2 + c^2) < 1$ for all $c^2 > 0$, the optimal shrinkage estimator *always* has a composite MSE less than $2$, the composite MSE of the ML estimator. Strictly speaking this estimator is **infeasible** since we don't know $c^2$. But it's a crucial step on our journey to make the leap from applying shrinkage to an estimator for a *single* unknown mean, to using the same idea for *more than one* unknown mean.
