...
series: 'A Good Instrument is a Bad Control'
series-label: 'Part II'
series-part-title: 'The general result, with simulations'
...
A lively discussion of bad controls ensued, during which I offered one of my favorite examples: [a good instrument is a bad control](/post/a-good-instrument-is-a-bad-control/).
...
More generally, a "bad control" is a covariate that we *shouldn't adjust for* when using a [selection-on-observables](/post/how-to-do-regression-adjustment/) approach to causal inference.
...
This post is a sequel, so you may find it helpful to glance at my [earlier post](/post/a-good-instrument-is-a-bad-control/) before reading further.
...
$$
where the error term $\eta$ satisfies $\text{Cov}(X,\eta) = \text{Cov}(Z,\eta) = \mathbb{E}(\eta) = 0$ [by construction](/post/why-econometrics-is-confusing-part-1-the-error-term/).
Further define the population linear regression of $X$ on $Z$, namely
$$
X = \pi_0 + \pi_Z Z + V
$$
where the error term $V$ satisfies $\text{Cov}(Z,V) = \mathbb{E}(V) = 0$ [by construction](/post/why-econometrics-is-confusing-part-1-the-error-term/).
...
$$
Using this notation, the result from [my earlier post](/post/a-good-instrument-is-a-bad-control/) can be written as
$$
...
To find out, we'll use the [FWL Theorem](/post/two-fwl-theorems-for-the-price-of-one/) as follows:
$$
...
Here the error term $\tilde{Z}$ satisfies $\mathbb{E}(\tilde{Z}) = \text{Cov}(\tilde{Z}, X) = 0$ [by construction](/post/why-econometrics-is-confusing-part-1-the-error-term/).
...
To make sure that we haven't made any silly mistakes, let's check our work using a little simulation experiment taken from my [earlier post](/post/a-good-instrument-is-a-bad-control/#a-simulation-example).
