...
In fact both of these prediction rules can be viewed as special cases of linear regression. Let $x_i$ denote student $i$'s score on midterm one and $y_i$ denote her score on midterm two. The sample mean $\bar{y} = \frac{1}{n} \sum_{i=1}^n y_i$ solves the optimization problem
$$
...
If $y$ is measured in inches, the SSR is measured in square inches; if $y$ is measured in kilometers, the SSR is measured in square kilometers.
...
We can see this in the expression for the standard error of the sample mean: $\text{SE}(\bar{X}) = \sigma_x/\sqrt{n}$.
