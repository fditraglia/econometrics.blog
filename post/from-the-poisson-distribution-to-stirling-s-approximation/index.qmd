...
```
As I discussed in an [earlier post](/post/street-fighting-numerical-analysis-part-1/), computers can only store a finite number of distinct numeric values.
...
$$
R even has a convenient, built-in function for evaluating the natural log of a factorial: `lfactorial()`.
...
Finally, and most importantly from my perspective, the heuristic argument I'll use below relies on none other than the [central limit theorem](/post/thirty-isn-t-the-magic-number/).
...
More generally, we can view a Poisson RV with rate parameter $n$ as the sum of $n$ iid Poisson(1) random variables.
By the [central limit theorem](/post/thirty-isn-t-the-magic-number/), it follows that
$$
...
Perhaps surprisingly, this approximation is extremely accurate even for fairly small values of $n$, as we can see by comparing it against `lfactorial()`.
```{r}
#| message: false
