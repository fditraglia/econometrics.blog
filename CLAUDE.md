...
**CI is pinned to Quarto 1.10.18**, matching the local install, so CI renders exactly what was rendered and checked locally — much of the theme keys on Quarto internals, and an unpinned CI would pick up new Quarto releases on its own schedule. To upgrade Quarto: bump the `version:` in `publish.yml` and the local install together, run `quarto render`, then both check scripts (`tools-check-layout.py`, `tools-check-folds.py`), and push only when all pass.
